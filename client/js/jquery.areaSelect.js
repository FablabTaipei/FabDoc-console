...
(function($, undefined) {
    ...
    AreaSelect.prototype.init = function() {
        var $canvas = $('<canvas/>');
        var offsetPosition = {
            top: this.$ele.position().top + parseInt(this.$ele.css("margin-top"), 10),
            left: this.$ele.position().left + parseInt(this.$ele.css("margin-left"), 10)
        }
        ...
        if (area != undefined) {
            ...
            for (var d in dragPoints) {
                if (near({
                        x: x,
                        y: y
                    }, dragPoints[d], this.options.padding)) {
                    nearDrag = true;
